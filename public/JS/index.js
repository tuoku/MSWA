...
searchBar.addEventListener('input', async ev => {
  ...
  if(val.length >= 3) {
    ...
    for (let i = 0; i < userSearchArray.length; i++) {
      ...
      if (userSearchArray[i].username.toString().substr(0, val.length - 1).toUpperCase() === val.replace('@','').toUpperCase()) {
        ...
        b.innerHTML += "<strong>@" + userSearchArray[i].username.toString().substr(0, val.length -1) + "</strong>";
        b.innerHTML += userSearchArray[i].username.toString().substr(val.length -1);
